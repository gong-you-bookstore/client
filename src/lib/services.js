...
export const getBooksByKakao = async (searchWord) => {
  return await axios({
    method: "get",
    url: `https://dapi.kakao.com/v3/search/book?&size=20&query=${searchWord}`,
    headers: {
...
export const getBookByNLK = async (isbn13) => {
  return await axios({
    method: "get",
    url: `https://www.nl.go.kr/NL/search/openApi/search.do?key=${process.env.REACT_APP_NLK_API_KEY}&apiType=json&kwd=${isbn13}&pageSize=1&category=도서`,
  })
};  